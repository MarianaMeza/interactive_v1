// This code was inspired by https://github.com/BMPMS/Titanic_scroll

var scrollVis = function () {
  // define constants (proportions copied from JV)
  // var width = 600;
  var width = 1000; //need a bigger width for my data
//  var widthsquare = 300
  var left_right_margin = 10;
  var top_bottom_margin = 30;
//  var height = 520;
  var height = 300; // need a bigger hegith for my data height. more heigt is less height
  ...
  // need to change the colors to match my palette and data
  ...
  // adapted sex, age
  var all_colours = { Men: '#2d7ac1', Women: '#7f1c32',homicides:"grey",
  "0-19":"#addd8e","20-29":"#78c679","30-44":"#41ab5d","45-69":"#238443","69 +":"#005a32",
  "Home": "#661b00", "Street":"#b23000","Other":"#ff7c4c","Firearm":"#238443"};
  ...
  var single_elements = function (my_data) {
    ...
    svg.append("svg:image")
      ...
      .attr("xlink:href", "images/title_image.png")
      .attr("height",389)
      .attr("width", 600)
      .attr("x", 0)
  ...
  var set_up_sections = function (my_data) {
    ...
    activateFunctions[1] = ["homicides","both",2000];   // all, survived not shown
    ...
    activateFunctions[5] = ["cause","both",2000]; // trying to show cause

  //  activateFunctions[5] = ["homicides","survived",2000]; // all, show survived
  ...
  chart.update = function (index, progress) {
    var show_image = [0]; //only show images on scroll index 1 and 5.
    //dont need the special image
     if(show_image.indexOf(index) >= 0){
        d3.select(".title_image").attr("visibility","visible");
      } else {
        d3.select(".title_image").attr("visibility","hidden");
      }
     activeIndex = index;
  ...
  var draw_dots = function (data_class, fill_type,transition){
    ...
    y_scale.domain([0,70]); // How tall the graphs are
    //set radius
    // var my_radius = 4.5;
    var my_radius = 2 // smaller radius
  //  var boxSize = 6; //size of each box sample dataset
    //var howManyAcross = Math.floor(widthsquare / boxSize);


//  boxGap = 0.1 // full dataset
    var boxGap = 3; //space between each box
    ...
    my_group.select(".bar_text")
            ...
            .attr("y",function(d){return y_scale(d3.max(my_data,function(m){if(m[data_class]==d){return m.row}})) - 12})
            ...
            .text(function(d){
              ...
              //  return group_count //if no survival, show no of passengers.
                  return parseInt(group_count/my_data.length*100) +"%" // This code gets the lables for graphs as percentages
              // } else {
...
d3.csv('data/homicide_sample.csv', display);

// need to adapt the data functions to show only 5, all with the 3,198 data points (one for every 10 homicides)
...
function convert_data(my_data){
  ...
  var all_per_row = 80; // needed more per row for my data
  var two_per_row = 40; // originaly was 20 I need et at 40
  var three_per_row = 28; // originaly was 14 I need et at 28
  var five_per_row = 20; // originaly was 10 I need et at 20

// need to adapt this for my variables

  var homicides = get_positions(my_data,all_per_row,[]); //stays the same since it is going throug every row
//var sex = get_positions(my_data,two_per_row,["male","female"],"Sex");
  var sex = get_positions(my_data,two_per_row,["Men","Women"],"Sex"); //adapted to mine
//var age = get_positions(my_data,five_per_row,[0,15,30,45,70],"Age");
  var age = get_positions(my_data,five_per_row,["0-19","20-29","30-44","45-69","69 +"],"Age");
  var cause = get_positions(my_data,two_per_row,["Firearm","Other"],"Cause");

//  var p_class = get_positions(my_data,three_per_row,[1,2,3],"Pclass");
  var p_class = get_positions(my_data,three_per_row,[1,2,3],"Pclass"); // instead of Pclass use Location

  var ch_1_2 = age_class(my_data,two_per_row);
  var w_ch_1_2 = women_children_class(my_data,two_per_row);

  return {homicides: homicides, sex: sex,age: age,p_class:p_class, cause:cause,ch_1_2:ch_1_2,w_ch_1_2:w_ch_1_2};
  ...
  //var p_class_labels = {1:"1st Class",2: "2nd Class",3:"3rd Class"};
    var p_class_labels = {1:"Home",2: "Street",3:"Other"};
    var positions = [];
    var band = "";
    var p_class="";
    var cause="";
        ...
        // if(field !== "Cause"){
          if(field !== "Age"){
            ...
            if(+v == (variables.length-1)){
              ...
              band = variables[v]; //only need the variables since they are already binned in categories
  ...
  function populate(my_data,band,p_class,col_per_row){
    ...
    var sex_labels = {"Men": "Men", "Women": "Women"}
    var cause_labels = {"Firearm": "Firearm", "Knife":"Knife", "Asphyxiation":"Asphyxiation", "Non-specified means":"Non-specified means", "Other":"Other"}
    ...
    for(d in my_data){
      if(isNaN(d) == false){
        ...
        current_positions.push ({
          ...
          age: band,
          cause:cause_labels[my_data[d].Cause],
          //cause: cause_labels[my_data[d].Cause],
