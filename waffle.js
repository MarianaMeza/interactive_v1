  ...
  const marginw = {top:10,right:200,bottom:30,left:100};
  const widthw  = 1000 - marginw.left - marginw.right;
  ...
  const howManyAcross = Math.floor(widthw / boxSize);
  ...
  var svg2 = d3.select("body")
        ...
        .attr("viewBox", "0 0 " + (widthw + marginw.left + marginw.right) + " " + (heightw + marginw.top + marginw.bottom))
    .append("svg:g")
        .attr("transform","translate(" + marginw.left + "," + marginw.top + ")");

        //Red and blue palette
 var colors = d3.scaleSequential(d3.interpolateReds)   // console.log(colors.domain)
  ...
  // d3.csv('homicide_full.csv').then(function(data, i){
     ...
     function CharVis (data, categoryHeading) {
      data.sort(function(a,b){ return d3.ascending(a[categoryHeading], b[categoryHeading])});
       ...
       const squares = svg2.selectAll("g.square")
           .data(data)
           squares.enter()
           .append("rect")
           .attr("class", "square")
